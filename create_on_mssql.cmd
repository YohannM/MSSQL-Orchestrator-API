...
sqlcmd -S %server% -d %db% -U %username% -P %pwd% -i config/ACTIVATE_OLE_AUTOMATION_COMPONENTS.sql
echo.
echo Checking Master Key :
sqlcmd -S %server% -d %db% -U %username% -P %pwd% -i config/CREATE_MASTER_KEY.sql
echo.
echo.
echo Table creation...
echo.
...
sqlcmd -S %server% -d %db% -U %username% -P %pwd% -i config/FILL_TABLE.sql
echo.
